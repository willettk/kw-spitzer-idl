...
; PURPOSE:
;
;	Measure the alphatic hydrocarbons in Spitzer IRS data
;
; INPUTS:
;
;	
...
;	fname = arr[k]
	...
	pivarr = [5.2,5.6,6.5,8.0,14.0,26.0]
	...
	bind685 = closetomed(wave,flux,6.79)
	eind685 = closetomed(wave,flux,6.88)
	...
	bind725 = closetomed(wave,flux,7.08)
	eind725 = closetomed(wave,flux,7.28)
	...
	print,''
	print,'6.85 optical depth for ',fname,' : ',string(-1d * minopt_685,format='(f6.2)')
	print,'6.85 int. flux for ',fname,'     : ',string(hacflux_685 * 1d21,format='(f8.2)')
	print,'7.25 optical depth for ',fname,' : ',string(-1d * minopt_725,format='(f6.2)')
	print,'7.25 int. flux for ',fname,'     : ',string(hacflux_725 * 1d21,format='(f8.2)')
