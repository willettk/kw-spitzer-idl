pro droopmed, specname, shsky = shsky, lhsky = lhsky, ss = ss, modules = modules, lores = lores
;+
; NAME:
;       DROOPMED
;
; PURPOSE:
;
; 	Median Spitzer droop images for all modules in order to remove cosmic rays and bad pixels 
;	and sky subtract
;
; INPUTS:
;
...
;
;	This program is largely based off of SPITZMED, and is directly called from there. 
;
;	Unlike SPITZMED, there are no peakup files for droop in ch0
;
; REVISION HISTORY
;       Written by K. Willett                July 2007
; 	Added MODULES input		- KW, Aug 07
;	Modified the counting scheme for LR files based on AOR number - KW, Nov 07
; 	Set default for no sky subtraction, added SHSKY and LHSKY keywords - KW, Mar 08
;	Routine is obsolete - incorporated loop for droop files into SPITZMED.pro - KW, Jul 08
;-
...
files_in_lh = fpath_lh + '*droop*fits'
...
if count_lh mod 2 ne 0 then print,'Wrong number of files in directory for droop LH module'
...
if keyword_set(lhsky) then begin
	...
	skyfiles_in_lh = skypath+'*droop*fits'
	
	skyfiles = findfile(skyfiles_in_lh)
	skycount = n_elements(skyfiles)
	
	if skycount mod 2 ne 0 then print,'Wrong number of files in directory for droop LH (no sky) module'
...
writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch3_sub_medarr_1p_droop.fits', sub_medarr_lh_1p, hdr_lh_1p
writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch3_sub_medarr_2p_droop.fits', sub_medarr_lh_2p, hdr_lh_2p
...
files_in_sh = fpath_sh + '*droop*fits'
...
if count_sh mod 2 ne 0 then print,'Wrong number of files in directory for droop SH module'
...
if keyword_set(shsky) then begin
	...
	skyfiles_in_sh = skypath+'*droop*fits'
	
	skyfiles = findfile(skyfiles_in_sh)
	skycount = n_elements(skyfiles)
	
	if skycount mod 2 ne 0 then print,'Wrong number of files in directory for droop SH (no sky) module'
	skyjunk = readfits(skyfiles(0), skyhdr, /silent)
	skysize = size(skyjunk)
	if skysize(0) ne 2 then begin
		print,'Not a 2D FITS file'
...
endif else begin	; NOTE - this means that the 'sub' part of the ch3 filenames is a blatant lie if /SHSKY is not set,
			; but it's not worth changing this in all downstream programs. 
...
writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch1_medarr_1p_droop.fits', sub_medarr_sh_1p, hdr_sh_1p
writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch1_medarr_2p_droop.fits', sub_medarr_sh_2p, hdr_sh_2p
...
files_in_ll = fpath_ll + '*droop*fits'
...
if keyword_set(ss) then begin
	...
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/ss/'+specname+'_ch2_sub_medarr_2o_1p_droop.fits', sub_medarr_ll2_1p, hdr_ll2_1p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/ss/'+specname+'_ch2_sub_medarr_2o_2p_droop.fits', sub_medarr_ll2_2p, hdr_ll2_2p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/ss/'+specname+'_ch2_sub_medarr_1o_1p_droop.fits', sub_medarr_ll1_1p, hdr_ll1_1p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/ss/'+specname+'_ch2_sub_medarr_1o_2p_droop.fits', sub_medarr_ll1_2p, hdr_ll1_2p
	...
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch2_sub_medarr_2o_1p_droop.fits', sub_medarr_ll2_1p, hdr_ll2_1p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch2_sub_medarr_2o_2p_droop.fits', sub_medarr_ll2_2p, hdr_ll2_2p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch2_sub_medarr_1o_1p_droop.fits', sub_medarr_ll1_1p, hdr_ll1_1p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch2_sub_medarr_1o_2p_droop.fits', sub_medarr_ll1_2p, hdr_ll1_2p
...
files_in_sl = fpath_sl + '*droop*fits'

files_sl=findfile(files_in_sl)		; Do not skip peakup files for droop
...
if n_elements(diff_dig_sl) mod 4 ne 0 then print,'Wrong number of files in directory for SL module'
...
if keyword_set(ss) then begin
	...
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/ss/'+specname+'_ch0_sub_medarr_2o_1p_droop.fits', sub_medarr_sl2_1p, hdr_sl2_1p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/ss/'+specname+'_ch0_sub_medarr_2o_2p_droop.fits', sub_medarr_sl2_2p, hdr_sl2_2p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/ss/'+specname+'_ch0_sub_medarr_1o_1p_droop.fits', sub_medarr_sl1_1p, hdr_sl1_1p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/ss/'+specname+'_ch0_sub_medarr_1o_2p_droop.fits', sub_medarr_sl1_2p, hdr_sl1_2p
	...
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch0_sub_medarr_2o_1p_droop.fits', sub_medarr_sl2_1p, hdr_sl2_1p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch0_sub_medarr_2o_2p_droop.fits', sub_medarr_sl2_2p, hdr_sl2_2p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch0_sub_medarr_1o_1p_droop.fits', sub_medarr_sl1_1p, hdr_sl1_1p
	writefits,'~/Astronomy/Research/Spitzer/spitzmed/'+specname+'_ch0_sub_medarr_1o_2p_droop.fits', sub_medarr_sl1_2p, hdr_sl1_2p
