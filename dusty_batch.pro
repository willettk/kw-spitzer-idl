pro dusty_batch, hightau = hightau
;+
; NAME:
;       
;	DUSTY_BATCH
...
; KEYWORDS:
;
;	HIGHTAU		- runs set of models on linear scale of tau_V from 100 to 500. Default
;				is on a logarithmic scale from 0.1 to 500. 
;
; EXAMPLE:
;
;	IDL> 
...
; 	Added HIGHTAU keyword			Feb 10
;-

starttime = systime(1)

Y_arr = [2, 5, 10, 15, 20, 30, 50, 75, 100, 150, 200, 300, 400, 500, 750, 1000]
...
for i = 0, n_elements(Y_arr) - 1 do begin
	...
	for j=0, n_elements(power_arr) - 1 do begin
		...
		file_delete, outfile
		file_delete, sppfile
		file_delete, dustydir+filename+'.s0'+string(indgen(10)+1,format='(i02)')
