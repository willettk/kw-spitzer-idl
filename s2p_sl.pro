pro s2p_sl, fname, nostop = nostop, color = color, coadd = coadd, ps = ps, lines = lines, $
...
if keyword_set(noclean) then files_opt = opath+'*'+fname+'*ch0*p_spect.tbl'$
	else files_opt = opath+'*'+fname+'*ch0*clean*spect.tbl'
...
trim_file = findfile('~/Astronomy/Research/Spitzer/'+dirtag+'/trim_templates/'+fname+'_lores.trim')
	if trim_file(0) eq '' then begin
		...
		sl1_edges = [trimstart(1),trimend(1)]	
...
allflux_1p   = [sl2_1p_flux, sl1_1p_flux]
allwave_1p   = [sl2_1p_wave, sl1_1p_wave]
allorder_1p  = [sl2_1p_order, sl1_1p_order]
allerror_1p  = [sl2_1p_error, sl1_1p_error]
allbit_1p    = [sl2_1p_bit, sl1_1p_bit]

allflux_2p   = [sl2_2p_flux, sl1_2p_flux]
allwave_2p   = [sl2_2p_wave, sl1_2p_wave]
allorder_2p  = [sl2_2p_order, sl1_2p_order]
allerror_2p  = [sl2_2p_error, sl1_2p_error]
allbit_2p    = [sl2_2p_bit, sl1_2p_bit]
...
allflux_coadd = [sl2_coadd, sl1_coadd]
...
if keyword_set(color) then begin
	if keyword_set(coadd) then begin
		oplot, sl1_1p_wave / (redshift + 1.),sl1_coadd, psym = 10, color = fsc_color('Red'), thick = lthick
		oplot, sl2_1p_wave / (redshift + 1.),sl2_coadd, psym = 10, color = fsc_color('Blue'), thick = lthick
		xyouts, 0.3, 0.9, /normal, 'Coadded', charsize = cs
		legend, ['LL1', 'LL2'], linestyle = [0,0], $
			color = [fsc_color('Red'), fsc_color('Blue')], $
		...
		legend, ['LL1, pos 1', 'LL2, pos 1', 'LL1, pos 2', 'LL2, pos 2'], $
			...
			/top, /left, charsize = cs, textcolors = defcolor, outline_color = defcolor
		oplot, sl1_1p_wave / (redshift + 1.), sl1_1p_flux, psym = 10, color = fsc_color('Red'), thick = lthick
		oplot, sl2_1p_wave / (redshift + 1.), sl2_1p_flux, psym = 10, color = fsc_color('Blue'), thick = lthick
		oplot, sl1_2p_wave / (redshift + 1.), sl1_2p_flux*offset, psym = 10, color = fsc_color('Red'), linestyle = 1, thick = lthick
		oplot, sl2_2p_wave / (redshift + 1.), sl2_2p_flux*offset, psym = 10, color = fsc_color('Blue'), linestyle = 1, thick = lthick
	endelse
endif else begin
	if keyword_set(coadd) then begin
