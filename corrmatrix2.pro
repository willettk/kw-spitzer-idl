pro corrmatrix2, stop=stop, spearman = spearman, verbose = verbose
;+
; NAME:
;       
;	CORRMATRIX2
...
; PURPOSE:
;
;	Create a correlation matrix of various properties measured for OHM galaxies; uses updated data
;		from DUSTY model fits
;
; INPUTS:
;
;
...
; Optical depth (tau_V) from DUSTY

otag = ohmdat('tag')
otauv = fltarr(n_elements(otag))
odtemp = fltarr(n_elements(otag))
...
; Dust temperature from DUSTY (see above)
...
writeohm = '~/Astronomy/Research/Spitzer/ohm/ohm_corrmatrix2_'+filetag+'.txt'
writecon = '~/Astronomy/Research/Spitzer/control/con_corrmatrix2_'+filetag+'.txt'
