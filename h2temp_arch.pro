pro h2temp_arch, verbose = verbose, ps = ps
;+
; NAME:
;       
;	H2TEMP_ARCH
...
h2 = {obj:strarr(n_elements(taglist)), tag:strarr(n_elements(taglist)), $
	temp_warm:fltarr(n_elements(taglist)), temp_hot:fltarr(n_elements(taglist)), $
	mass_warm:fltarr(n_elements(taglist)), mass_hot:fltarr(n_elements(taglist)), $
...
for k=0,1 do begin
	...
	filenames = ['~/Astronomy/Research/Spitzer/papers/h2temp_arch1.ps',$
		'~/Astronomy/Research/Spitzer/papers/h2temp_arch2.ps']
	
	if keyword_set(ps) then begin
		set_plot,'ps'
		device, filename=filenames[k], /portrait
		tempcs = 0.5
		axescs = 1
		labelcs = 0.5
		defthick = 3
		...
		if n_elements(where(h2fluxarr ne 0)) gt 1 then begin
			...
			if h2fluxarr[4] ne 0 then $
				arrow, ej[4], alog((h2fluxarr[4]/ (aj[4] * del_ej[4])) / gj[4]), $
					ej[4], alog((h2fluxarr[4]/ (aj[4] * del_ej[4])) / gj[4]) - 2, $
			...
			if n_elements(hwarm) gt 1 then begin
				...
				xyouts, 3000, -3, 'T!Iwarm!N = '+string(temp_warm, format = '(i3)')+' K', $
					charsize = tempcs, charthick = cthick
			...
			print,'Completed H2TEMP_ARCH for '+fname
...
cd,'~/Astronomy/Research/Spitzer/archived'
save, h2, filename='~/Astronomy/Research/Spitzer/archived/h2.sav'
