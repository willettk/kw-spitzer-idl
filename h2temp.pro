pro h2temp, verbose = verbose, ps = ps, silent = silent, stop = stop
;+
; NAME:
;       
;	H2TEMP
...
; 	Revised to make figs. for paper		Aug 08
; 	Added hot H2 mass			Aug 08
; 	Combine ARCH and MEGA samples 		Oct 08
;-
...
objlist = [transpose(ohmobj),transpose(archobj)]
taglist = [transpose(ohmtag),transpose(archtag)]
dllist = [transpose(ohmdl),transpose(archdl)]
...
for k=0,2 do begin
	...
	h2filename = '~/Astronomy/Research/Spitzer/papers/h2temp'+string(k+1,format='(i1)')+'.ps'
...
;		if k eq 2 and j eq n_elements(objlist) - 36 then j = 17		; Kluge to make three plots
	endfor
...
cd,'~/Astronomy/Research/Spitzer/ohm'
save, h2, filename='~/Astronomy/Research/Spitzer/ohm/h2.sav'
