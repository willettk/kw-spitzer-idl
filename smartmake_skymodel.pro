...
pro smartmake_skymodel, fname, ps = ps, write = write, nobonus = nobonus, $
...
restore,'~/Astronomy/Research/Spitzer/archived/skymodel.sav'

; Rebin the model to match the wave array

shwave_005 = wave_005(shind_005)
sh_subtract = fltarr(n_elements(shwave))

for i= 0, n_elements(shwave) - 1 do begin
	temp1 = closeto(shwave_005,shwave(i))
	flux1 = shsky_005(temp1)
	sh_subtract(i) = flux1
endfor
...
sh_subtract_bin  = fltarr(n_elements(wavearr))
...
for i = 0, n_elements(wavearr)-1 do begin
	...
	sh_subtract_bin(i) = sh_subtract(closeto(shwave,wavearr(i)))
endfor
...
caltot_shift_sub = fltarr(nsteps)
for i = 0, nsteps - 1 do begin
	tempcal = shfluxbin + (2d * i/float(nsteps) - 1d) * firstguess_shift
	caltot_shift(i) = total(((tempcal - slfluxbin)/slerrbin)^2)
	tempcal_sub = shfluxbin - sh_subtract_bin + (2d * i/float(nsteps) - 1d) * firstguess_shift
	caltot_shift_sub(i) = total(((tempcal_sub - slfluxbin)/slerrbin)^2)
endfor

ji_shift = where(caltot_shift eq min(caltot_shift))
hr_shift = (2d * ji_shift(0) / nsteps - 1d) * firstguess_shift
ji_shift_sub = where(caltot_shift_sub eq min(caltot_shift_sub))
hr_shift_sub = (2d * ji_shift_sub(0) / nsteps - 1d) * firstguess_shift
...
tempind42 = n_elements(wavearr) / 2
...
oplot,wavearr,shfluxbin - sh_subtract_bin, color=fsc_color("Yellow")
oplot,wavearr,shfluxbin - sh_subtract_bin + hr_shift_sub, color=fsc_color("Purple")
