...
; PURPOSE:
;
;	Extract the offset peakup fluxes from a subset of targets in the non-masing control sample
;
; CATEGORY:
...
filelist=['~/Astronomy/Research/Spitzer/control/data/control0??_spectra/ch0/bcd/*_000[0,1]_000[0,1,2,3]_*bcd.fits']

files=file_search(filelist)


; Cull list for duplicates

newfiles = strarr(n_elements(files))
m = 0
for k = 0, n_elements(files) - 1 do begin
	temptag = strmid(files(k), 56, 10)
	tempindex = where(temptag eq confiles)
...
dir_out='~/Astronomy/Research/Spitzer/control/pu_control/'
fitsdir_out='~/Astronomy/Research/Spitzer/control/pu_control/fits/'
phot_file='~/Astronomy/Research/Spitzer/control/pu_control/pu_control_phot.txt'
centroid_file='~/Astronomy/Research/Spitzer/control/pu_control/pu_control_cntrds.txt'
...
;        object=STRJOIN(STRSPLIT(STRJOIN(STRSPLIT(STRJOIN(STRSPLIT(STRJOIN(STRSPLIT(STRJOIN(STRSPLIT(object_long,'[', /extract),''),']', /extract),''),'/', /extract),'_'),/extract),''), '?',/extract),'')
	...
	nameout=progid+'_'+STRMID(object,0,18)+'_'+STRTRIM(filter,2)+'_'+STRMID(newfiles(i), 56, 10)
...
;		print, 'Average flux = ',string(mean([flux_0, flux_1]),format='(f5.2)'),' mJy'
                ...
                printf, 5,  progid, aorkey, date, tag, object, filter, grptime, xcen_0, ycen_0, mags_0, sky_0, flux_0, $
                  format='(A7, 4X, I8, 3X, A12, 3X,A11, 3X, A14, 3X,A4, 2X, F6.3, 4X, F6.2, 2X, F6.2, 2X, F10.1, 2X, F6.0, F7.2)'
		...
		if ideg_0 ge 0 then $
                printf, 6, progid, 'ACQ_'+strn(aorkey), date, tag, object, filter, xcen_0, ycen_0, ihr_0, imin_0, xsec_0, ideg_0, imn_0, xsc_0, $
                  format='(A7, 4X, A12, 3X, A12, 3X, A11, 3X, A14, 3X,A4, 2X, F6.2, 4X, F6.2, 3X, i02,":", i02, ":", f05.2, 4X, i02,":", i02, ":", f05.2)' $
		else $
                printf, 6, progid, 'ACQ_'+strn(aorkey), date, tag, object, filter, xcen_0, ycen_0, ihr_0, imin_0, xsec_0, ideg_0, imn_0, xsc_0, $
                  format='(A7, 4X, A12, 3X, A12, 3X, A11, 3X, A14, 3X,A4, 2X, F6.2, 4X, F6.2, 3X, i02,":", i02, ":", f05.2, 4X, i03,":", i02, ":", f05.2)'
...
; Write out the files after modifying the header to fix the WCS bug of S10.5
        ...
        fxaddpar, hdr_0, 'HISTORY', 'Created by pu_control program of V. Charmandaris/K. Willett '
        ...
        fxaddpar, hdr_1, 'HISTORY', 'Created by pu_control program of V. Charmandaris/K. Willett '
...
; Save the average of the two fluxes to an IDL file
    ...
    save, file='~/Astronomy/Research/Spitzer/control/data/idl_sav/pu_fluxes.sav', $
